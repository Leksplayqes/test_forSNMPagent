                    ...
                    for value in OIDdescr[device][serive]:
                        for descr in OIDdescr[device][serive][value]:
                            if descr in name.prettyPrint():
                                parcrecive = '%s = %s' % (
                                    value + OIDdescr[device][serive][value][descr] + "." + name.prettyPrint().replace(descr, ""),
...
def start_snmp_listener_in_background():
    """Запускает SNMP listener в фоновом потоке"""
    snmp_thread = threading.Thread(target=run_snmp_trap_listener, daemon=True)
    snmp_thread.start()
    return snmp_thread


if __name__ == "__main__":
    # Запуск SNMP listener в фоне
    snmp_thread = start_snmp_listener_in_background()
